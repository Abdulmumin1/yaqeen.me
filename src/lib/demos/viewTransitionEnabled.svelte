<script>
	...
	var dataValues = [5, 7, 29, 18, 10, 23, 30, 13];
	var speed = 200;
	...
	function renderChart(arr) {
		...
		const transition = document.startViewTransition(async () => {
			data = arr;
			await tick();
			// console.log('view-transition-api not working for me oh');
		});
	...
	async function startAnim() {
		// restart();
		await bubbleSortWithDelay(data).then((sortedArray) => {
...
{#if useAnimateFlip}
	<blockquote class="mt-2">
		* your browser does not support <code>view-transtions-api</code>, using
		<code>svelte animaite:flip</code> to simulate behaviour 🫶🏽
	</blockquote>
{/if}
